...
class Part(Model):
    ...
    def _suggested_config(self, Lmin: float, Lmax:float) -> tuple[float, int]:
        ...
        for the generation of reciprocal spacing. Based on
        the fact that for example, for most of the case,
        q range needed for a d=100 sphere is within (0.001, 0.5).
        So this function will calculate optimal real_spacing and
        n_s value for the range considering the calculation
        resourse used.
        the s values after fft is generated by fftgreq, approximate
        smin (>0) = 1/(n_s*d), smax = 1/(2d) where s = q/(2pi)
        So the values calculated are
        n_s = 1000, d = pi*L/50
        But must ensure that the lattice number in Lmin is lerger
        than 10.
        ...
        '''
        n_s = 1000
        real_spacing = 1 * torch.pi * Lmax / 50
