...
class Part(Model):
    ...
    def _suggested_config(self, Lmin: float, Lmax:float) -> tuple[float, int]:
    ...
    def gen_lattice_meshgrid(self, spacing: float | None = None) -> tuple[Tensor, Tensor, Tensor]:
        ...
        if spacing is None:
            ...
            spacing = self._suggested_config(Lmin, Lmax)[0]
    ...
    def gen_reciprocal_lattice(self, n_s: int | None = None, need_centering: bool = True) -> Tensor:
        ...
        if n_s is None:
            Lmin = min(xmax-xmin, ymax-ymin, zmax-zmin)
            Lmax = max(xmax-xmin, ymax-ymin, zmax-zmin)
            n_s = self._suggested_config(Lmin, Lmax)[1]
